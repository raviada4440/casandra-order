...
type PatientWithPartialRelationsWithAction = PatientWithPartialRelations & {
  action?: string
}

// Column Definitions
const columnHelper = createColumnHelper<PatientWithPartialRelationsWithAction>()


const PatientListTable = ({ patientsData, columnFiltersData }: { patientsData: PatientWithPartialRelations[], columnFiltersData: ColumnFiltersState }) => {
  ...
  const columns = useMemo<ColumnDef<PatientWithPartialRelationsWithAction, any>[]>(
    () => [
      columnHelper.accessor('FirstName', {
        header: 'First Name',
        cell: ({ row }) => <Typography className={`${classes.cell250}`}>{row.original.FirstName}</Typography>
      }),
      columnHelper.accessor('LastName', {
        header: 'Last Name',
        cell: ({ row }) => <Typography className={`${classes.cell250}`}>{row.original.LastName}</Typography>
      }),
      columnHelper.accessor('DateOfBirth', {
        ...
        cell: ({ row }) => {
          ...
          if (row.original.DateOfBirth) {
            ...
            formattedDate = `${birthDate.getMonth() + 1}/${birthDate.getDate()}/${birthDate.getFullYear()}`;
          }
      ...
      columnHelper.accessor('Gender', {
        header: 'Gender',
        cell: ({ row }) => <Typography className={`${classes.cell250}`}>{row.original.Gender}</Typography>
      }),
      columnHelper.accessor('Email', {
        header: 'Email',
        cell: ({ row }) => <Typography className={`${classes.cell250}`}>{row.original.Email}</Typography>
      }),
      columnHelper.accessor('Mobile', {
        header: 'Cell Phone',
        cell: ({ row }) => <Typography className={`${classes.cell250}`}>{row.original.Mobile}</Typography>
      }),
      columnHelper.accessor('action', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center'>
            <Link href={getLocalizedUrl(`apps/patients/edit/${row.original.Id}`, locale as Locale)} className='flex'>
  ...
  const table = useReactTable({
    data: data as PatientWithPartialRelations[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
  ...
  useEffect(() => {
    // const filteredData = patientsData?.filter(test => {
    ...
    <Card>
      ...
      <div className='flex flex-col sm:flex-row is-full sm:is-auto items-center gap-2'>
        <Button
          ...
          href={getLocalizedUrl('/apps/patients/add', locale as Locale)}
